...
static inline void mcount_memcpy1(void *restrict dst, const void *restrict src,
				  int len)
{
...
static inline void mcount_memcpy4(void *restrict dst, const void *restrict src,
				  int len)
{
...
extern enum filter_result
mcount_entry_filter_check(struct mcount_thread_data *mtdp, unsigned long child,
...
extern struct uftrace_mmap *new_map(const char *path, uint64_t start,
				    uint64_t end, const char *prot);
...
int read_pmu_event(struct mcount_thread_data *mtdp, enum uftrace_event_id id,
		   void *buf);
void release_pmu_event(struct mcount_thread_data *mtdp,
