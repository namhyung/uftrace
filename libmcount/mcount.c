...
static const struct sigtable {
	...
	SIGTABLE_ENTRY(SIGHUP),	 SIGTABLE_ENTRY(SIGINT),
	SIGTABLE_ENTRY(SIGQUIT), SIGTABLE_ENTRY(SIGILL),
	SIGTABLE_ENTRY(SIGTRAP), SIGTABLE_ENTRY(SIGABRT),
	SIGTABLE_ENTRY(SIGBUS),	 SIGTABLE_ENTRY(SIGFPE),
	SIGTABLE_ENTRY(SIGKILL), SIGTABLE_ENTRY(SIGUSR1),
	SIGTABLE_ENTRY(SIGSEGV), SIGTABLE_ENTRY(SIGUSR2),
	SIGTABLE_ENTRY(SIGPIPE), SIGTABLE_ENTRY(SIGALRM),
	SIGTABLE_ENTRY(SIGTERM), SIGTABLE_ENTRY(SIGSTKFLT),
	SIGTABLE_ENTRY(SIGCHLD), SIGTABLE_ENTRY(SIGCONT),
	SIGTABLE_ENTRY(SIGSTOP), SIGTABLE_ENTRY(SIGTSTP),
	SIGTABLE_ENTRY(SIGTTIN), SIGTABLE_ENTRY(SIGTTOU),
	SIGTABLE_ENTRY(SIGURG),	 SIGTABLE_ENTRY(SIGXCPU),
	SIGTABLE_ENTRY(SIGXFSZ), SIGTABLE_ENTRY(SIGVTALRM),
	SIGTABLE_ENTRY(SIGPROF), SIGTABLE_ENTRY(SIGWINCH),
	SIGTABLE_ENTRY(SIGIO),	 SIGTABLE_ENTRY(SIGPWR),
...
static int parse_sigspec(char *spec, struct uftrace_filter_setting *setting)
{
	...
	*pos = '@';
	...
	tmp = NULL;
	if (setup_trigger_action(spec, &tr, &tmp, TRIGGER_FL_SIGNAL, setting) <
	    0)
		return -1;
			       ...
			       bool force)
{
	...
	if (argument_str || retval_str || autoargs_str ||
	    (trigger_str &&
	     (strstr(trigger_str, "arg") || strstr(trigger_str, "retval")))) {
...
static void send_session_msg(struct mcount_thread_data *mtdp,
			     const char *sess_id)
{
...
void mtd_dtor(void *arg)
{
	...
	tmsg.pid = getpid(), tmsg.tid = mcount_gettid(mtdp),
...
static void segv_handler(int sig, siginfo_t *si, void *ctx)
{
	...
	while (rstack >= mtdp->rstack) {
		...
		pr_warn("[%d] (%s[%lx] <= %s[%lx])\n", idx--, cname,
			rstack->child_ip, pname, rstack->parent_ip);
...
static void mcount_init_file(void)
{
	...
	pr_dbg("new session started: %.*s: %s\n", SESSION_ID_LEN,
	       mcount_session_name(), basename(mcount_exename));
...
#define FLAGS_TO_CHECK                                                         \
	(TRIGGER_FL_DEPTH | TRIGGER_FL_TRACE_ON | TRIGGER_FL_TRACE_OFF |       \
	 TRIGGER_FL_TIME_FILTER)
...
static int script_save_context(struct script_context *sc_ctx,
			       struct mcount_thread_data *mtdp,
			       struct mcount_ret_stack *rstack, char *symname,
			       bool has_arg_retval, struct list_head *pargs)
{
			      ...
			      struct uftrace_trigger *tr)
{
	...
	if (script_save_context(&sc_ctx, mtdp, rstack, symname,
				tr->flags & TRIGGER_FL_ARGUMENT, tr->pargs) < 0)
		goto skip;
...
#define FLAGS_TO_CHECK                                                         \
	(TRIGGER_FL_FILTER | TRIGGER_FL_RETVAL | TRIGGER_FL_TRACE |            \
	 TRIGGER_FL_FINISH | TRIGGER_FL_CALLER)
...
#define FLAGS_TO_CHECK                                                         \
	(TRIGGER_FL_RECOVER | TRIGGER_FL_TRACE_ON | TRIGGER_FL_TRACE_OFF)
		...
		if (tr->flags & FLAGS_TO_CHECK) {
			...
			if (tr->flags &
			    (TRIGGER_FL_TRACE_ON | TRIGGER_FL_TRACE_OFF))
				mtdp->enable_cached = mcount_enabled;
		}
	}
...
#define FLAGS_TO_CHECK                                                         \
	(MCOUNT_FL_FILTERED | MCOUNT_FL_NOTRACE | MCOUNT_FL_RECOVER)
...
static int __cygprof_entry(unsigned long parent, unsigned long child)
{
	 ...
	 */
	tr.flags &=
		~(TRIGGER_FL_ARGUMENT | TRIGGER_FL_RETVAL | TRIGGER_FL_RECOVER);
...
TEST_CASE(mcount_estimate_return_depth)
{
	...
	for (i = 0; i < ARRAY_SIZE(test_scenario); i++) {
		TEST_EQ(mcount_entry(test_scenario[i].frame_pointer,
				     test_scenario[i].func_addr, NULL),
			0);

		pr_dbg("[%d] mcount entry: idx = %d, depth = %d\n", i,
		       mtdp->idx, mtdp->rstack[mtdp->idx - 1].depth);
