...
void __close_data_file(struct opts *opts, struct uftrace_data *handle,
		       bool unload_modules);
static inline void close_data_file(struct opts *opts,
...
struct uftrace_session *find_task_session(struct uftrace_session_link *sess,
					  ...
					  uint64_t timestamp);
void create_task(struct uftrace_session_link *sess,
...
void walk_tasks(struct uftrace_session_link *sess, walk_tasks_cb_t callback,
		void *arg);
...
void send_trace_info(int sock, struct uftrace_file_header *hdr, void *info,
		     int len);
