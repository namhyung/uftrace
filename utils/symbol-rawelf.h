...
#define elf_get_symbol(elf, iter, idx)                                         \
	memcpy(&(iter)->sym, &(iter)->data[idx * (iter)->ent_size],            \
	       (iter)->ent_size)
...
#define elf_for_each_dynamic(elf, iter)                                        \
	for (elf_get_secdata((elf), (iter)),                                   \
	     ...
	     (iter)->ent_size = (iter)->shdr.sh_entsize, (iter)->i = 0,        \
	     (iter)->nr = (iter)->shdr.sh_size / (iter)->ent_size;             \
	     (iter)->type == SHT_DYNAMIC && (iter)->i < (iter)->nr &&          \
	     memcpy(&(iter)->dyn, &(iter)->data[(iter)->i * (iter)->ent_size], \
		    (iter)->ent_size);                                         \
	     (iter)->i++)
...
#define elf_for_each_symbol(elf, iter)                                         \
	for (elf_get_secdata((elf), (iter)),                                   \
	     ...
	     memcpy(&(iter)->sym, &(iter)->data[(iter)->i * (iter)->ent_size], \
		    (iter)->ent_size);                                         \
	     (iter)->i++)
...
#define elf_for_each_rel(elf, iter)                                            \
	for (elf_get_secdata((elf), (iter)),                                   \
	     ...
	     memcpy(&(iter)->rel, &(iter)->data[(iter)->i * (iter)->ent_size], \
		    (iter)->ent_size);                                         \
	     (iter)->i++)
...
#define elf_for_each_note(elf, iter)                                           \
	for (elf_get_secdata((elf), (iter)),                                   \
	     ...
	     memcpy(&(iter)->nhdr, (iter)->data + (iter)->i,                   \
		    sizeof((iter)->nhdr)) &&                                   \
	     ((iter)->note_name =                                              \
		      (iter)->data + (iter)->i + sizeof((iter)->nhdr)) &&      \
	     ((iter)->note_desc =                                              \
		      (iter)->note_name + ALIGN((iter)->nhdr.n_namesz, 4));    \
