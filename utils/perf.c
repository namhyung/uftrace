		      ...
		      const char *dirname, int use_ctxsw)
{
	...
	for (cpu = 0; cpu < nr_cpu; cpu++) {
		...
		perf->page[cpu] =
			mmap(NULL, PERF_MMAP_SIZE, PROT_READ | PROT_WRITE,
			     MAP_SHARED, fd, 0);
		...
		snprintf(filename, sizeof(filename), "%s/perf-cpu%d.dat",
			 dirname, cpu);
...
again:
	...
	case PERF_RECORD_COMM:
		...
		if (fread(&u.c.sample_id, sizeof(u.c.sample_id), 1, perf->fp) !=
		    1)
			return -1;
...
void update_perf_task_comm(struct uftrace_data *handle)
{
	...
	for (i = 0; i < handle->nr_perf; i++) {
		...
		while (!perf->done) {
			...
			memcpy(task->comm, perf->u.comm.comm,
			       sizeof(task->comm));
		}
...
static void remove_event_rstack(struct uftrace_task_reader *task)
{
	...
	do {
		last = list_last_entry(&task->event_list.read, typeof(*last),
				       list);
...
void process_perf_event(struct uftrace_data *handle)
{
	...
	while (1) {
		...
		} else if (perf->type == PERF_RECORD_SWITCH &&
			   !perf->u.ctxsw.out) {
			struct uftrace_rstack_list_node *last;
			uint64_t delta;
