		    ...
		    struct uftrace_record *rec)
{
	...
	case EVENT_ID_WATCH_CPU:
		if (read_task_event_size(task, &u.watch.cpu,
					 sizeof(u.watch.cpu)) < 0)
			return -1;
		if (task->h->needs_byte_swap)
	...
	case EVENT_ID_WATCH_ADDR:
		if (data_is_lp64(task->h)) {
			if (read_task_event_size(task, &u.watch.w64,
						 sizeof(u.watch.w64)) < 0)
				return -1;
			if (task->h->needs_byte_swap) {
			...
			if (read_task_event_size(task, &u.watch.w32,
						 sizeof(u.watch.w32)) < 0)
				return -1;
			if (task->h->needs_byte_swap) {
