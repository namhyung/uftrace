		       ...
		       bool needs_symtab, bool sym_rel_addr, bool needs_srcline)
{
	...
	while (getline(&line, &sz, fp) >= 0) {
		if (!strncmp(line, "TASK", 4)) {
			num = sscanf(line + 5,
				     "timestamp=%lu.%lu tid=%d pid=%d", &sec,
				     &nsec, &tmsg.tid, &tmsg.pid);
			if (num != 4)
				goto out;
			...
			num = sscanf(line + 5,
				     "timestamp=%lu.%lu pid=%d ppid=%d", &sec,
				     &nsec, &tmsg.tid, &tmsg.pid);
			if (num != 4)
				goto out;
			...
			num = sscanf(line + 5,
				     "timestamp=%lu.%lu %*[^i]id=%d sid=%s",
				     &sec, &nsec, &smsg.task.pid,
				     (char *)&smsg.sid);
			if (num != 4)
				goto out;
...
int read_events_file(struct uftrace_data *handle)
{
	...
	while (getline(&line, &sz, fp) >= 0) {
		...
		if (!strncmp(line, "EVENT", 5)) {
			sscanf(line + 7, "%u %[^:]:%s", &evt_id, provider,
			       event);
			...
			const char *exename)
{
	...
	fprintf(fp, "SESS timestamp=%s pid=%d sid=%s exename=\"%s\"\n", ts,
		smsg->task.pid, smsg->sid, exename);
		       ...
		       const char *libname)
{
	...
	fprintf(fp,
		"DLOP timestamp=%s tid=%d sid=%s base=%" PRIx64
		" libname=\"%s\"\n",
