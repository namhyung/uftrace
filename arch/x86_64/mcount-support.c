			    ...
			    struct uftrace_arg_spec *spec)
{
	...
	case ARG_TYPE_INDEX:
		...
		break;
	case ARG_TYPE_FLOAT:
		reg_idx = spec->idx + UFT_X86_REG_FLOAT_BASE;
		break;
	case ARG_TYPE_STACK:
	default:
		return -1;
	...
	switch (reg_idx) {
	case UFT_X86_REG_RDI:
		ctx->val.i = ARG1(regs);
		break;
	case UFT_X86_REG_RSI:
		ctx->val.i = ARG2(regs);
		break;
	case UFT_X86_REG_RDX:
		ctx->val.i = ARG3(regs);
		break;
	case UFT_X86_REG_RCX:
		ctx->val.i = ARG4(regs);
		break;
	case UFT_X86_REG_R8:
		ctx->val.i = ARG5(regs);
