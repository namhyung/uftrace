...
TEST_CASE(dynamic_x86_handle_lea)
{
	...
	cs_insn *insn = NULL;
	cs_x86 *x86;
	uint8_t lea_insn[7] = { 0x48, 0x8d, 0x05,
				0x60 }; /* lea 0x60(%rip),%rax */
	...
	count = cs_disasm(disasm.engine, lea_insn, sizeof(lea_insn), info.addr,
			  0, &insn);
	TEST_EQ(count, 1);
	...
	count = cs_disasm(disasm.engine, new_insns, new_size, CODEPAGE_BASE, 0,
			  &insn);
	TEST_EQ(count, 1);
...
TEST_CASE(dynamic_x86_handle_mov_load)
{
	...
	cs_insn *insn = NULL;
	cs_x86 *x86;
	uint8_t mov64_insn[7] = { 0x48, 0x8b, 0x3d,
				  0x64 }; /* mov 0x64(%rip),%rdi */
