...
이름
====
uftrace-record - 명령어를 실행하고 추적한 데이터를 기록한다.


사용법
========
uftrace record [*옵션*] COMMAND [*명령어-옵션*]


설명
===========
이 명령어는 COMMAND를 실행하며 함수의 추적 데이터들을 모으고,
아무것도 출력하지 않은 채 uftrace data 디렉토리에 그 내용들을 저장한다. 
이 데이터는 이후 `uftrace replay`나 `uftrace report`를 통해 검사될 수 있다.
...
:   SIZE 바이트보다 큰 함수들을 동적으로 패치한다. 동적 추적은 *DYNAMIC TRACING*을 참고한다.
...
\--port=*PORT*
:   (`-H`를 이용해)데이터를 네트워크로 보낼 때, 기본 포트 (8090)가 아닌 다른 포트를 사용한다.
